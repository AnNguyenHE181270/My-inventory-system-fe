...
const menuItems = [
  ...
  { path: dashboardRoutes.staffExport, label: 'Xuất kho', icon: 'fa-solid fa-truck-ramp-box' },
...
const pageTitles = {
  ...
  [dashboardRoutes.staffExport]: 'Xuất kho',
...
function SalesLayout() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans">
      <aside
        className={`fixed left-0 top-0 z-40 flex h-screen flex-col border-r border-gray-200 bg-white shadow-sm transition-all duration-300 ${
          collapsed ? 'w-16' : 'w-60'
        }`}
      >
              ...
              return (
                <Link
                  ...
                  className={`flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition ${
                    active ? 'bg-red-50 text-red-600 shadow-sm' : 'text-gray-700 hover:bg-gray-50'
                  }`}
                >
      ...
      <div className={`min-h-screen transition-all duration-300 ${collapsed ? 'ml-16' : 'ml-60'}`}>
